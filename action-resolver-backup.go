...
// 			return
// 		}
// 		color := msg.PropertyColor
// 		targetPlayerID := msg.TargetPlayerID
// 		if color == "" || targetPlayerID == "" {
// 			return
// 		}
...
// 				pd.TargetColor = color
// 				pd.TargetPlayer = player
// 				room.Game.PushNewMessage(fmt.Sprintf("%s requested %s property set from %s", p.Name, color, player.Name))
...
// 					"action": "clear",
// 				})
// 				room.Game.HandleDealBreaker(room.Game.CurrentTurn, &pd.TargetPlayer.Player, pd.TargetColor)
// 				room.Game.PushNewMessage(fmt.Sprintf("Deal Breaker: %s stole %s property set from %s", room.Game.CurrentTurn.Name, pd.TargetColor, p.Name))
...
// 		if player, ok := room.Players[targetPlayerID]; ok {
// 			pd.TargetPlayer = player
// 			if card, ok, color, commit := pd.TargetPlayer.GetProperty(cardID); ok {
// 				if color != c.Wild && pd.TargetPlayer.HasCompleteSet(color) {
// 					return
// 				}
// 				pd.TargetColor = color
// 				pd.TargetCard = card
...
// 			if msg.Type == "continue" {
// 				pd.TargetCommit()
// 				room.Game.CurrentTurn.AddToProperty(c.Wild, pd.TargetCard)
// 				room.SetPlayerActionMessage(room.Players[room.Game.CurrentTurn.ID], map[string]string{
// 					"type":   "action",
// 					"action": "clear",
// 				})
...
// 			if card, ok, color, commit := pd.TargetPlayer.GetProperty(pd.TargetCardID); ok {
// 				if color != c.Wild && pd.TargetPlayer.HasCompleteSet(color) {
// 					return
// 				}
...
// 		if card, ok, color, commit := p.GetProperty(pd.PlayerCardID); ok {
// 			if color != c.Wild && p.HasCompleteSet(color) {
// 				return
// 			}
...
// 				pd.PlayerCommit()
// 				pd.TargetCommit()
// 				pd.TargetPlayer.AddToProperty(c.Wild, pd.PlayerCard)
// 				room.Game.CurrentTurn.AddToProperty(c.Wild, pd.TargetCard)
// 				room.PendingAction = nil
// 				fmt.Println("Done")
// 				room.SetPlayerActionMessage(room.Players[room.Game.CurrentTurn.ID], map[string]string{
...
// 			if card, ok, color, commit := pd.TargetPlayer.GetProperty(msg.CardID); ok {
// 				if pd.TargetPlayer.AddToProperty(msg.PropertyArrangeDestColor, card) {
// 					commit()
// 					room.Game.PushNewMessage(fmt.Sprintf("%s moved %s card from %s to %s", pd.TargetPlayer.Name, card.GetName(), color, msg.PropertyArrangeDestColor))
// 				}
// 			}
// 			return
// 		}
...
// 				room.Game.CurrentTurn.AddToProperty(c.Wild, card)
// 			}
...
// 		if msg.PropertyColor == "" || (pd.IsForOnePlayer && msg.TargetPlayerID == "") {
// 			// give both at once
// 			return
// 		}
...
// 		color := msg.PropertyColor
// 		if color != "" {
// 			if pd.RentCard != nil && room.Game.DoColorMatchRentColors(color, pd.RentCard) {
// 				pd.BasePayable, _ = p.GetRentOnColor(color)
...
// 			})
// 			if pd.IsForOnePlayer {
// 				room.Game.PushNewMessage(fmt.Sprintf("%s will be expecting $%v rent on %s property from %s", p.Name, uint8(pd.BasePayable)*pd.RentMultiplier, color, pd.TargetPlayer.Name))
// 			} else {
// 				room.Game.PushNewMessage(fmt.Sprintf("%s will be expecting $%v rent on %s property from everyone", p.Name, uint8(pd.BasePayable)*pd.RentMultiplier, color))
// 			}
